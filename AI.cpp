...
{
	int score = boardEvaluation->evaluateBoard(board, false);
	...
	if (isMaximizingPlayer)
	{
		...
		for (int i = 0; i < moves.size(); i++)
		{
			...
			int minimaxValue = minimax(board, depth + 1, false) - depth;
	...
	else 
	{
		...
		{
			board.addMark(!isMaximizingPlayer, moves[i].row, moves[i].col);
			int minimaxValue = minimax(board, depth + 1, true) + depth;
